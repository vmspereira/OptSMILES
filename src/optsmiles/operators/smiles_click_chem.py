...
from optsmiles.operators.MolObjectHandling import (try_deprotanation,
                                                   check_sanitization, 
                                                   try_reprotanation, 
                                                   handle_frag_check, 
...
class SmilesClickChem(object):
    ...
    def retrieve_functional_group_dict(self, rxn_library, function_group_library):
        ...
        SmartClickChem reactions are retrieved.
    ...
    def run_smiles_click(self, ligand_smiles_string):
        ...
        mol_reprotanated = try_reprotanation(mol_reprotanated)
        if mol_reprotanated is None:
            return None

        mol_deprotanated = copy.deepcopy(mol)
        mol_deprotanated = try_deprotanation(mol_deprotanated)
                ...
                for i in range(0, len(fun_groups_in_rxn)):
                    if i == contains_group:
                        ...
                        for find_mol_tries in range(0, 100):
                            ...
                            # try sanitizing, which is necessary later
                            comp_mol = check_sanitization(comp_mol)

                            # Try with deprotanated molecule rdkit to
                            # recognize for the reaction
                            comp_mol = try_deprotanation(comp_mol)
                            if comp_mol is None:
                                continue
        ...
        if is_rxn_complete is True:
            reaction_product = check_sanitization(reaction_product)
    ...
    def check_if_product_is_good(self, reaction_product):
        ...
        reaction_product = handle_frag_check(reaction_product)
        if reaction_product is None:
            return None

        # Make sure there are no unassigned atoms which made it through. These
        # are very unlikely but possible
        reaction_product = check_for_unassigned_atom(reaction_product)
        if reaction_product is None:
            return None

        reaction_product = try_reprotanation(reaction_product)
        if reaction_product is None:
            return None

        # Remove H's
        reaction_product = try_deprotanation(reaction_product)
        if reaction_product is None:
            return None
